...
DEFAULT_COLOR = (255, 255, 255) # White for fallback
...
class MapRenderer:
    ...
    def draw(self, game_state, tiled_data, camera):
        # The main draw call in game.py should handle screen clearing and flipping
        # This method should only draw game elements onto the provided surface (self.screen)
        ...
        # Optional: Draw collision debug if needed
        # self.draw_collision_debug(game_state, camera)

    def draw_path(self, game_state, camera):
        """Draw the path with camera-relative positions"""
        # Draw onto the main screen surface directly, assuming transparency is handled
        if game_state.path:
            for x, y in game_state.path:
                world_x = x * GRID_SIZE + GRID_SIZE // 2
                world_y = y * GRID_SIZE + GRID_SIZE // 2
                screen_x, screen_y = camera.apply((world_x, world_y))
                # Draw semi-transparent circles for the path
    ...
    def draw_player(self, game_state, camera):
        """Draw the player with camera-relative position and form-specific color."""
        screen_x, screen_y = camera.apply(game_state.player_pixel_pos)
        
        # Get the color based on the current form
        player_color = FORM_COLORS.get(game_state.current_form, DEFAULT_COLOR)
        
        pygame.draw.circle(
            self.screen, 
            player_color, # Use the form-specific color
    ...
    def draw_tile_layer(self, layer, tile_width, tile_height, camera):
        ...
        if not layer_width or not layer_height or not layer_data:
            return # Skip layer if data is missing
    ...
    def draw_collision_debug(self, game_state, camera):
        ...
        for y in range(height):
            for x in range(width):
                # Draw red overlay if NOT walkable for the CURRENT form
